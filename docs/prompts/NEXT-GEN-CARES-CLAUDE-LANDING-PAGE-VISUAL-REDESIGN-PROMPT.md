...
> redesign described below was already implemented directly on `main`
> (commits `22f6451`, `9978e0b`, `c922dfa`) without first producing the
> mandatory Section 21 pre-implementation audit or the Section 24
> post-implementation report, and without an explicit human approval
> checkpoint between them. No functional/business-logic change is evidenced
> in that implementation. Use this prompt now to **close that gap**: perform
> the retroactive audit against the code as it stands, verify the
> Section 27 Definition of Done items that have not yet been evidenced
> (WCAG 2.2 AA, responsive, FR/NL, `pnpm exec task ci`), and produce the
> Section 24 final engineering report before any further landing-page visual
> work continues. Do not silently treat the existing implementation as
> pre-approved.
...
> Produce the retroactive Section 21 audit and the Section 24 final
> engineering report for the landing-page visual redesign already merged to
> `main`, run the repository-native quality gates, and report gaps against
> the Section 27 Definition of Done. Do not implement further visual changes
> beyond what is strictly required to close a gap this audit finds, and stop
> for human approval before doing so.
...
# 26. CLAUDE CODE BEHAVIOR REQUIREMENTS
